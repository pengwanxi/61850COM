...
void ShowRTDBInfo()
{
	...
	if( g_pRTDBObj )
	{
		printf("\n Memory Address : 0x%x", (unsigned long)g_pRTDBObj->m_pRTDBSpace);
		...
		printf("\n DZ      Sum = %d", g_pRTDBObj->m_nDZSum);
	}
	else
	{
		printf("\n Invalid RTDB Object !");
	}
	printf("\n----All Copy Right By ESD ----\n");
}
...
int Create_SHM_DBase(char* szPath, int nExtLen)
{
    ...
    printf("1113\n");
	//�������ݿ⹲���ڴ�
   if( g_pRTDBObj->CreateRTDBObj(szText) == -1 )
	{
    printf("1114\n");
		delete g_pRTDBObj;
		g_pRTDBObj = NULL;
...
int Open_SHM_DBase()
{
	...
	if( !g_pRTDBObj ) return -1;
	//���ӹ����ڴ����ݿ�
	if( g_pRTDBObj->OpenRTDBObj(szText) == -1 )
	{
		delete g_pRTDBObj;
		g_pRTDBObj = NULL;
		return -2;
...
int Get_RTDB_Extend(long *addr)
{
	if( !g_pRTDBObj ) return -1;
	if( !g_pRTDBObj->m_pRTDBSpace ) return -2;
	*addr = (long)&g_pRTDBObj->m_pRTDBSpace->pExtAddr;
...
{
	if (!g_pRTDBObj) return NULL;
	return g_pRTDBObj->GetDzObj(wStn, wPnt);
}
