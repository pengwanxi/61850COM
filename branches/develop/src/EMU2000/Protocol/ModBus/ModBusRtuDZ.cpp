//CModBusRtuDz.cpp: implementation of the CModBusRtuDz class.
//////////////////////////////////////////////////////////////////////
...
CModBusRtuDz::~CModBusRtuDz()
{/*{{{*/
	printf( "Delete CModBusRtuDz bus = %d , Addr = %d \n" , m_byLineNo , m_wDevAddr );
	yk_info.clear();
	modbus_conf.clear();
}/*}}}*/

BOOL CModBusRtuDz::GetProtocolBuf( BYTE * buf , int &len , PBUSMSG pBusMsg )
{/*{{{*/
	...
	if ((modbus_conf[pos].func == 0xFF) || (modbus_conf[pos].registe == 0xEEEE)) //��¼�¿��ٱ��� ��һ���������ĵ�λ��
	{
		pos_flag = pos;

	}
	while (1)//��ת����һ���ǿ��ٱ��� ͨѶ�����ڼ�������
	{
		if ((modbus_conf[pos].func == 0xFF) || (modbus_conf[pos].registe == 0xEEEE))
		{
			pos = (pos + 1) % line;
	...
	if (modbus_conf[pos].type == 0)//Ҫ���͵Ŀ��ٱ���
	{

		SendQuicklyBuf(modbus_conf[pos], buf, len, pBusMsg);
		pos = (pos + 1) % line;
		pos_flag = pos;
	}
	else
	{
		...
		SendRtuBuf(modbus_conf[pos],buf, len, pBusMsg); //Ҫ���͵���ͨRtu����
...
BOOL CModBusRtuDz::GetDzWriteProcess(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
...
void CModBusRtuDz::ProcessPatchDz(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
...
void CModBusRtuDz::ProcessDzRead(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
	WORD wDataNum = pBusMsg->DataNum;
	...
	//WORD wDzPnt = da_dz_valid->wPnt;
	 WORD wDzPnt = dz_data->wPnt;
	float fDz[100] = { 0 };
	BYTE byRowNum = 0;
	BYTE byDataNum = wDataNum - 1;
	int i;

	printf("******byDataNum=%d******startorder=%d*****readval_pos_num=%d*\n", byDataNum, dz_data->wPnt, readval_pos_num);//ԭ���ϲ��ܿ籨��
	//wDzPnt ������ʼ���  byDataNum���Ƕ�ֵ����
	if (wDzPnt >= readval_info[readval_pos_num - 1].start_num&& byDataNum <= readval_info[readval_pos_num - 1].get_num)//�ȼ���·��Ķ�ֵ��ʼ����ǲ����ڹ��������õ����һ������
	{
		byRowNum = readval_pos_num - 1;

	}
	else
	{
		for (i = 0; i < readval_pos_num - 1; i++)
		{
			if (wDzPnt >= readval_info[i].start_num&&wDzPnt < readval_info[i + 1].start_num&& byDataNum <= readval_info[i].get_num)
				byRowNum = i;
			else{
				printf("***Cross message ***\n");
				//�籨���˳�����;
			}
		}
	...
	DzStartOrder = wDzPnt;//��ֵ����ʼ��ţ�TCI�·��ģ�
	DzMount = byDataNum;//��ֵ��������TCI�·��ģ�

	BYTE index = 0;


	buf[index++] = m_wDevAddr;  //�豸��ַ
	buf[index++] = modbusconf.func; //������

	buf[index++] = modbusconf.registe >> 8;//�Ĵ�����ַ
	buf[index++] = modbusconf.registe;

	buf[index++] = modbusconf.registe_num >> 8;//�Ĵ�������
	buf[index++] = modbusconf.registe_num;

	WORD wCRC = GetCrc(buf, index);//У��
...
void CModBusRtuDz::ProcessDzWrite(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
	WORD wDataNum = pBusMsg->DataNum;
	...
	{
		printf("++++%d+++\n", writeval_info[i].start_num);
		if (wDzPnt != writeval_info[i].start_num)
			continue;
		if (byDataNum == 1 && writeval_info[i].type == 6)
			byRowNum = i;
		else if (byDataNum >1 && writeval_info[i].type == 7)
			byRowNum = i;
		else
			continue;
...
void CModBusRtuDz::ProcessSingleDz(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
...
void CModBusRtuDz::GetDzWriteBuffer( BYTE byRowNum , BYTE * buf , int &len )
{
	BYTE pos = byRowNum;
...
BOOL CModBusRtuDz::ProcessProtocolBuf( BYTE * buf , int len )
{/*{{{*/
	if (m_bUnvanish)
	{
	...
	if ((modbus_conf[pos_flag].func == 0xFF) || (modbus_conf[pos_flag].registe == 0xEEEE))
	{
		//���ٱ����жϽ��յı����Ƿ���ȷ
		if (!WhetherQuicklyBufValue(buf, len))
		{
			return FALSE;
		}
		else
	...
	{
			ResolveRtuBuf(buf,len);
	}

	return TRUE ;
}/*}}}*/

void CModBusRtuDz::ProcessUnvarnishRtn(BYTE * buf, int len)
{
	//�˴����ж�10������͸��У��
...
void  CModBusRtuDz::ModBusWriteDz_10(unsigned char *buffer, MODBUSCONF modbusconf)
{
...
BOOL CModBusRtuDz::Init( BYTE byLineNo )
{/*{{{*/
	//�����ʼ��ģ������
	char szFileName[256] = "";
...
//ң���źŴ���
BOOL CModBusRtuDz::GetYKBuffer( BYTE * buf , int &len , PBUSMSG pBusMsg )
{/*{{{*/
	int i = 0;
	YK_DATA *yk_data;
...
//�ַ���תʮ�����ƣ���ȡ�����ļ�ʱʹ��
unsigned int CModBusRtuDz::Atoh(char *buf)
{/*{{{*/
	BYTE i = 0;
	UINT tempvalue = 0;
	UINT value = 0;
...
//�������ļ�ĳ�г�������ʱ������л���Ĭ�����ã���Ĭ�����ã��ɷ�����������
void CModBusRtuDz::DefaultValConfig(MODBUSCONF * mc)
{/*{{{*/
	mc->type = 1;
	mc->func = 2;
	mc->registe = 0x0064;
...
//��ȡ�����ļ�
int CModBusRtuDz::ReadConf(char *filename)
{/*{{{*/
	FILE *hFile;
	char szText[160];
	char *temp;
	...
	while( fgets(szText, sizeof(szText), hFile) != NULL )
	{
		...
		if(temp == NULL)
			continue;
		if( ( atoi(temp) >= 0 ) && ( atoi(temp) <= 10) )//�������� 0��ʾ���ٱ���
			mc.type = atoi(temp);
		else
		{
			conflag = 1;
		...
		else if( i < 18)
		{
			...
			case 15:mc.SetTimeFlag = 0;
			case 16:mc.SoeFlag = 0;
			case 17:mc.YkSelFlag = 0;
			case 18:mc.YkExctFlag = 0;
			/*lel*/
			case 19:mc.YxProcessMethod = 0;
			case 20:mc.YxByBitValue = 0;break;
			/*end*/
			}
		}
	...
	for(i=0;i<num;i++)
	{
		...
		if( modbus_conf[i].type == 6 || modbus_conf[i].type == 7 )
		{
			...
			writeval.type = modbus_conf[i].type;
...
int CModBusRtuDz::TimePackMsecBigEdian(MODBUSCONF modbusconf,unsigned char *buffer,int i,struct tm *p,WORD msec)
{/*{{{*/
...
int CModBusRtuDz::TimePackMsecLittleEdian(MODBUSCONF modbusconf,unsigned char *buffer,int i,struct tm *p,WORD msec)
{/*{{{*/
	buffer[i++] = modbusconf.registe_num*2;
	buffer[i++] = (p->tm_year)-100;
	buffer[i++] = 1+(p->tm_mon);
...
int CModBusRtuDz::TimePackIEC(MODBUSCONF modbusconf, unsigned char *buffer, int i, struct tm *p, WORD msec)
{
	printf("-----%d---msec=%d--\n", p->tm_sec, msec);
	long sec_msec = 0;
...
//
//ʱ�����
int CModBusRtuDz::SysLocalTime(MODBUSCONF modbusconf,unsigned char *buffer,int i)
{/*{{{*/
	struct timeval tv;
	struct timezone tz;
...
void CModBusRtuDz::Esl411SoeSendBuf(BYTE * buf ,int *len)
{/*{{{*/
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
	buf[ index++ ] = 0x0c;
...
//ң�ţ�ң�⣬��ʱ�ȷ��ͱ���
void CModBusRtuDz::SendBuf( MODBUSCONF modbusconf, BYTE * buf ,int *len )
{ /*{{{*/
	BYTE index = 0;
...
//����ң��Ԥ�ñ���
void CModBusRtuDz::YkPresetSendBuf( MODBUSCONF modbusconf , YK_DATA *yk_data , BYTE * buf ,int *len )
{/*{{{*/
#if 0
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
...
//����ң�ر���
void CModBusRtuDz::YkJ05SendBuf( MODBUSCONF modbusconf , YK_DATA *yk_data , BYTE * buf ,int *len )
{/*{{{*/
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
	buf[ index++ ] = modbusconf.func;
...
//����ң�ر���
void CModBusRtuDz::YkSendBuf( MODBUSCONF modbusconf , YK_DATA *yk_data , BYTE * buf ,int *len )
{/*{{{*/
#if 0
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
...
void CModBusRtuDz::ReadvalSendBuf( MODBUSCONF modbusconf , BYTE * buf ,int *len )
{/*{{{*/
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
	buf[ index++ ] = modbusconf.func;
...
void CModBusRtuDz::WritevalSendBuf( MODBUSCONF modbusconf , unsigned int * val , BYTE * buf ,int *len )
{/*{{{*/
	int index = 0;
	buf[ index++ ] = m_wDevAddr;
	buf[ index++ ] = modbusconf.func;
...
//���ݳ��ȣ����ݸ�ʽ����
short CModBusRtuDz::TwoByteValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{/*{{{*/
	short val = 0;
	unsigned char buf[2] = "";
	//�����봦
...
//���ݳ��ȣ����ݸ�ʽ����
unsigned short CModBusRtuDz::TwoByteValue_unsigned(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{/*{{{*/
...
//���ݳ��ȣ����ݸ�ʽ����
int CModBusRtuDz::FourByteValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{/*{{{*/
	int val = 0;
	int i1, i2, i3, i4;
	unsigned char buf[4] = "";
...
//���ݳ��ȣ����ݸ�ʽ����
unsigned int CModBusRtuDz::FourByteValue_unsigned(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{/*{{{*/
	unsigned int val = 0;
	int i1, i2, i3, i4;
	unsigned char buf[4] = "";
...
int CModBusRtuDz::GetBCD(BYTE byData)
{
	return  (byData / 16) * 10 + byData % 16;
}

//�з���Ϊfloat�����ݸ�ʽ����
float CModBusRtuDz::FloatValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{/*{{{*/
	float val = 0;
...
double CModBusRtuDz::DoubleValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
	double val = 0;
...
unsigned long long CModBusRtuDz::EightByteValue_unsigned(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{/*{{{*/
...
//���ݸ�ʽ���з���
double CModBusRtuDz::ModBusValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{/*{{{*/
	float value_f = 0;
	int value_i = 0;
...
//ң�Ŵ���
void CModBusRtuDz::ModBusYxDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
...
UINT CModBusRtuDz::ModBusYXTempValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{/*{{{*/
	int value_i = 0;
	unsigned int value_ui = 0;
	short value_s = 0;
...
//ң�Ű�λ����
void CModBusRtuDz::ModBusYxBitDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	WORD i = 0;
	WORD j = 0;
...
/// \����:	modbusconf
// --------------------------------------------------------
void CModBusRtuDz::ModBusYxByteDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	WORD i = 0;
	WORD j = 0;
	BYTE pos = modbusconf.skew_byte;
...
void CModBusRtuDz::ModBusYxByteDeal_new(unsigned char *buffer, MODBUSCONF modbusconf)
{
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
	WORD start_num = modbusconf.start_num;
...
void CModBusRtuDz::ModBusRsl_411YxYcDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE Fun= buffer[1];
	switch( Fun )
	{
...
//ң�⴦��
void CModBusRtuDz::ModBusYcDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
	WORD start_num = modbusconf.start_num;
...
void CModBusRtuDz::ModBusJ05YkDeal(unsigned char *buffer,MODBUSCONF modbusconf )
{/*{{{*/
...
//ң�ش���
void CModBusRtuDz::ModBusRtuYkDeal(unsigned char *buffer,MODBUSCONF modbusconf )
{/*{{{*/
	WORD i = 0;
	BYTE ykVal= 2;
	BYTE pos = modbusconf.skew_byte ;
...
void CModBusRtuDz::ModBusYkDeal(unsigned char *buffer,MODBUSCONF modbusconf )
{/*{{{*/
...
//ң������
void CModBusRtuDz::ModBusYmDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
...
//��ʱ����
void CModBusRtuDz::ModBusSetTime(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE pos = modbusconf.skew_byte ;
	WORD get_num = modbusconf.get_num;
...
void CModBusRtuDz::ModBusRtuSoeDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE pos = modbusconf.skew_byte;
	//WORD start_num = modbusconf.start_num;
	int i = 0;
...
void CModBusRtuDz::ModBusYZ202SoeDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE pos = modbusconf.skew_byte;
	//WORD start_num = modbusconf.start_num;
	//int i = 0;
...
void CModBusRtuDz::ModBusEsl_411SoeDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD wPnt = 0;
	BYTE soeflag = 0;
...
//SOE����
void CModBusRtuDz::ModBusSoeDeal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
...
void CModBusRtuDz::ModBusReadVal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	int pos = modbusconf.skew_byte;
	int get_num = modbusconf.get_num;
	int i = 0;
	...
	printf("order=%d mount=%d\n",DzStartOrder,DzMount);

	printf("modbusconf.sign=%d modbusconf.data_len=%d pos=%d \n", modbusconf.sign, modbusconf.data_len,pos);
	printf("%d %s\n", __LINE__, __FILE__);
	for(i=0;i<get_num;i++)
	{
		float wVal = 0 ;
	...
	if (DzMount <= get_num)
	{
		printf("%d %s\n", __LINE__, __FILE__);
		//��ȡ��ʼ���
		memcpy(data2, data + DzStartOrder - modbusconf.start_num, sizeof(DZ_DATA)*DzMount);
		m_pMethod->SetDzCallRtn(this, bySrcBusNo, wSrcDevAddr, 0, data2, DzMount);
		printf("%d %s\n", __LINE__, __FILE__);
...
void CModBusRtuDz::ModBusWriteVal(unsigned char *buffer,MODBUSCONF modbusconf)
{/*{{{*/
	int pos = modbusconf.skew_byte;
	int get_num = modbusconf.get_num;
...
BOOL CModBusRtuDz::GetDevCommState( )
{/*{{{*/
...
BOOL CModBusRtuDz::SendRtuBuf(MODBUSCONF modbusconf,BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	...
	if (pBusMsg != NULL)
	{
		...
		else if (pBusMsg->byMsgType == DZ_PROTO)
		{
			...
			printf("*********%d %s****** byBusNo=%d ******\n", __LINE__, __FILE__, pBusMsg->SrcInfo.byBusNo);
...
void CModBusRtuDz::ProcessUnvarnish(BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	if (pBusMsg == NULL)
		return;
...
BOOL CModBusRtuDz::SendQuicklyBuf(MODBUSCONF modbusconf,BYTE * buf, int &len, PBUSMSG pBusMsg)
{
	BYTE index = 0;
	if (DevCirFlag == FALSE)
	{
...
BOOL CModBusRtuDz::ResolveQuicklyBuf(BYTE * buf, int len)
{
	buf[2] = modbus_conf[pos_flag].registe_num * 2;//�Ĵ�������*2���ֽڣ������ֻ��Կ��ٱ��Ļظ����ĵĽ��� ��ÿһ�����ݳ��ȼ�buf[2] �������õļĴ������� �������¶���
...
BOOL CModBusRtuDz::ResolveRtuBuf(BYTE *buf, int len)
{
	BYTE temp_flag = 0;
	BYTE CRC_flag = 0;
	WORD wCRC = 0;

	/*printf("**%d recv:", pos_flag);
	for (int i = 0; i < len; i++)
	{
		printf("%02x ", buf[i]);
	}
...
*--------------------------------------------------------------------------------------
*       Class: CModBusRtuDz
*      Method:  WhetherQuicklyBufValue
* Description:  �鿴���ձ�����Ч��(���ٱ���)
*       Input:  ������ ����
*		Return:  BOOL
*--------------------------------------------------------------------------------------
*/
BOOL CModBusRtuDz::WhetherQuicklyBufValue(BYTE *buf, int &len)
{
	//BYTE *pointer = buf;
	//WORD wCrc;
	//char szBuf[256];
	...
	//	{
	//		sprintf(szBuf, "Fast message recv addr err recv Addr=%d LocalAddr=%d\n", *pointer, m_wDevAddr);
	...
	//	if (*(pointer + 2) > len - 3)
	//	{
	//		sprintf(szBuf, "Fast message recv len err recv len=%d\n", *(pointer + 2));
	//		OutBusDebug(m_byLineNo, (BYTE *)szBuf, strlen(szBuf), 2);
	//		goto DEFAULT;
	//	}
	//	//�ж�У��
	...
	//	{
	//		sprintf(szBuf, "Fast message recv crc err recvcrc=%.2x%.2x, localcrc=%.2x%.2x\n",
