// CModBusRtuAndQuickly.cpp: implementation of the CModBusRtuAndQuickly class.
//////////////////////////////////////////////////////////////////////
...
CModBusRtuAndQuickly::CModBusRtuAndQuickly()
{ /*{{{*/
	pos_flag = -1;
	pos = 0;
	settime_pos = 0;
...
CModBusRtuAndQuickly::~CModBusRtuAndQuickly()
{ /*{{{*/
	printf("Delete CModBusRtuAndQuickly bus = %d , Addr = %d \n", m_byLineNo, m_wDevAddr);
	yk_info.clear();
	modbus_conf.clear();
} /*}}}*/

BOOL CModBusRtuAndQuickly::GetProtocolBuf(BYTE *buf, int &len, PBUSMSG pBusMsg)
{ /*{{{*/
...
BOOL CModBusRtuAndQuickly::GetDzWriteProcess(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
...
void CModBusRtuAndQuickly::ProcessPatchDz(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
...
void CModBusRtuAndQuickly::ProcessDzRead(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
	WORD wDataNum = pBusMsg->DataNum;
...
void CModBusRtuAndQuickly::ProcessDzWrite(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
	WORD wDataNum = pBusMsg->DataNum;
...
void CModBusRtuAndQuickly::ProcessSingleDz(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	DZ_DATA *dz_data;
	dz_data = (DZ_DATA *)pBusMsg->pData;
...
void CModBusRtuAndQuickly::GetDzWriteBuffer(BYTE byRowNum, BYTE *buf, int &len)
{
	BYTE pos = byRowNum;
...
BOOL CModBusRtuAndQuickly::ProcessProtocolBuf(BYTE *buf, int len)
{ /*{{{*/
	printf("-----------recv----------------\n");
	for (int i = 0; i < len; i++)
		printf("%02x  ", buf[i]);
...
void CModBusRtuAndQuickly::ModBusWriteDz_10(unsigned char *buffer, MODBUSCONF modbusconf)
{
...
BOOL CModBusRtuAndQuickly::Init(BYTE byLineNo)
{ /*{{{*/
	// �����ʼ��ģ������
	char szFileName[256] = "";
...
//
// ��վ״̬
void CModBusRtuAndQuickly::TimerProc()
{ /*{{{*/
	time_t temp_time;
	time(&temp_time);
...
// ң���źŴ���
BOOL CModBusRtuAndQuickly::GetYKBuffer(BYTE *buf, int &len, PBUSMSG pBusMsg)
{ /*{{{*/
	int i = 0;
	YK_DATA *yk_data;
...
// �ַ���תʮ�����ƣ���ȡ�����ļ�ʱʹ��
unsigned int CModBusRtuAndQuickly::Atoh(char *buf)
{ /*{{{*/
	BYTE i = 0;
	UINT tempvalue = 0;
	UINT value = 0;
...
// �������ļ�ĳ�г�������ʱ������л���Ĭ�����ã���Ĭ�����ã��ɷ�����������
void CModBusRtuAndQuickly::DefaultValConfig(MODBUSCONF *mc)
{ /*{{{*/
	mc->type = 1;
	mc->func = 2;
	mc->registe = 0x0064;
...
// ��ȡ�����ļ�
int CModBusRtuAndQuickly::ReadConf(char *filename)
{ /*{{{*/
	FILE *hFile;
	char szText[160];
	char *temp;
...
int CModBusRtuAndQuickly::TimePackMsecBigEdian(MODBUSCONF modbusconf, unsigned char *buffer, int i, struct tm *p, WORD msec)
{ /*{{{*/
...
int CModBusRtuAndQuickly::TimePackMsecLittleEdian(MODBUSCONF modbusconf, unsigned char *buffer, int i, struct tm *p, WORD msec)
{ /*{{{*/
	buffer[i++] = modbusconf.registe_num * 2;
	buffer[i++] = (p->tm_year) - 100;
	buffer[i++] = 1 + (p->tm_mon);
...
int CModBusRtuAndQuickly::TimePackIEC(MODBUSCONF modbusconf, unsigned char *buffer, int i, struct tm *p, WORD msec)
{
	printf("-----%d---msec=%d--\n", p->tm_sec, msec);
	long sec_msec = 0;
...
//
// ʱ�����
int CModBusRtuAndQuickly::SysLocalTime(MODBUSCONF modbusconf, unsigned char *buffer, int i)
{ /*{{{*/
	struct timeval tv;
	struct timezone tz;
...
void CModBusRtuAndQuickly::Esl411SoeSendBuf(BYTE *buf, int *len)
{ /*{{{*/
	BYTE index = 0;
	buf[index++] = m_wDevAddr;
	buf[index++] = 0x0c;
...
// ң�ţ�ң�⣬��ʱ�ȷ��ͱ���
void CModBusRtuAndQuickly::SendBuf(MODBUSCONF modbusconf, BYTE *buf, int *len)
{ /*{{{*/
	BYTE index = 0;
...
// ����ң��Ԥ�ñ���
void CModBusRtuAndQuickly::YkPresetSendBuf(MODBUSCONF modbusconf, YK_DATA *yk_data, BYTE *buf, int *len)
{ /*{{{*/
#if 0
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
...
// ����ң�ر���
void CModBusRtuAndQuickly::YkJ05SendBuf(MODBUSCONF modbusconf, YK_DATA *yk_data, BYTE *buf, int *len)
{ /*{{{*/
	BYTE index = 0;
	buf[index++] = m_wDevAddr;
	buf[index++] = modbusconf.func;
...
// ����ң�ر���
void CModBusRtuAndQuickly::YkSendBuf(MODBUSCONF modbusconf, YK_DATA *yk_data, BYTE *buf, int *len)
{ /*{{{*/
#if 0
	BYTE index = 0;
	buf[ index++ ] = m_wDevAddr;
...
void CModBusRtuAndQuickly::ReadvalSendBuf(MODBUSCONF modbusconf, BYTE *buf, int *len)
{ /*{{{*/
	BYTE index = 0;
	buf[index++] = m_wDevAddr;
	buf[index++] = modbusconf.func;
...
void CModBusRtuAndQuickly::WritevalSendBuf(MODBUSCONF modbusconf, unsigned int *val, BYTE *buf, int *len)
{ /*{{{*/
	int index = 0;
	buf[index++] = m_wDevAddr;
	buf[index++] = modbusconf.func;
...
// ���ݳ��ȣ����ݸ�ʽ����
short CModBusRtuAndQuickly::TwoByteValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{ /*{{{*/
	short val = 0;
	unsigned char buf[2] = "";
	// �����봦
...
unsigned short CModBusRtuAndQuickly::TwoByteValue_unsigned(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{ /*{{{*/
...
// ���ݳ��ȣ����ݸ�ʽ����
int CModBusRtuAndQuickly::FourByteValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
	int val = 0;
	int i1, i2, i3, i4;
	unsigned char buf[4] = "";
...
// ���ݳ��ȣ����ݸ�ʽ����
unsigned int CModBusRtuAndQuickly::FourByteValue_unsigned(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
	unsigned int val = 0;
	int i1, i2, i3, i4;
	unsigned char buf[4] = "";
...
int CModBusRtuAndQuickly::GetBCD(BYTE byData)
{
	return (byData / 16) * 10 + byData % 16;
}

// �з���Ϊfloat�����ݸ�ʽ����
float CModBusRtuAndQuickly::FloatValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
	float val = 0;
...
unsigned long long CModBusRtuAndQuickly::EightByteValue_unsigned(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
...
double CModBusRtuAndQuickly::DoubleValue(unsigned char *buffer, int a, MODBUSCONF modbusconf)
{ /*{{{*/
	double val = 0;
...
// ���ݸ�ʽ���з���
double CModBusRtuAndQuickly::ModBusValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{ /*{{{*/
	float value_f = 0;
	int value_i = 0;
...
// ң�Ŵ���
void CModBusRtuAndQuickly::ModBusYxDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
...
UINT CModBusRtuAndQuickly::ModBusYXTempValue(unsigned char *buffer, int pos, MODBUSCONF modbusconf)
{ /*{{{*/
	int value_i = 0;
	unsigned int value_ui = 0;
	short value_s = 0;
...
// ң�Ű�λ����
void CModBusRtuAndQuickly::ModBusYxBitDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	WORD i = 0;
	WORD j = 0;
...
/// \����:	modbusconf
// --------------------------------------------------------
void CModBusRtuAndQuickly::ModBusYxByteDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	WORD i = 0;
	WORD j = 0;
	BYTE pos = modbusconf.skew_byte;
...
void CModBusRtuAndQuickly::ModBusYxByteDeal_new(unsigned char *buffer, MODBUSCONF modbusconf)
{
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
	WORD start_num = modbusconf.start_num;
...
void CModBusRtuAndQuickly::ModBusRsl_411YxYcDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE Fun = buffer[1];
	switch (Fun)
	{
...
// ң�⴦��
void CModBusRtuAndQuickly::ModBusYcDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
	WORD start_num = modbusconf.start_num;
...
void CModBusRtuAndQuickly::ModBusJ05YkDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
...
// ң�ش���
void CModBusRtuAndQuickly::ModBusRtuYkDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	WORD i = 0;
	BYTE ykVal = 2;
	BYTE pos = modbusconf.skew_byte;
...
void CModBusRtuAndQuickly::ModBusYkDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
...
// ң������
void CModBusRtuAndQuickly::ModBusYmDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	/*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
...
// ��ʱ����
void CModBusRtuAndQuickly::ModBusSetTime(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD get_num = modbusconf.get_num;
...
void CModBusRtuAndQuickly::ModBusRtuSoeDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE pos = modbusconf.skew_byte;
	// WORD start_num = modbusconf.start_num;
	int i = 0;
...
void CModBusRtuAndQuickly::ModBusYZ202SoeDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE pos = modbusconf.skew_byte;
	// WORD start_num = modbusconf.start_num;
	// int i = 0;
...
void CModBusRtuAndQuickly::ModBusEsl_411SoeDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	BYTE pos = modbusconf.skew_byte;
	WORD wPnt = 0;
	BYTE soeflag = 0;
...
// SOE����
void CModBusRtuAndQuickly::ModBusSoeDeal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
...
void CModBusRtuAndQuickly::ModBusReadVal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	int pos = modbusconf.skew_byte;
	int get_num = modbusconf.get_num;
	int i = 0;
...
void CModBusRtuAndQuickly::ModBusWriteVal(unsigned char *buffer, MODBUSCONF modbusconf)
{ /*{{{*/
	int pos = modbusconf.skew_byte;
	int get_num = modbusconf.get_num;
...
BOOL CModBusRtuAndQuickly::GetDevCommState()
{ /*{{{*/
...
BOOL CModBusRtuAndQuickly::SendRtuBuf(MODBUSCONF modbusconf, BYTE *buf, int &len, PBUSMSG pBusMsg)
{
...
BOOL CModBusRtuAndQuickly::SendQuicklyBuf(MODBUSCONF modbusconf, BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	BYTE index = 0;
	if (DevCirFlag == FALSE)
	{
...
BOOL CModBusRtuAndQuickly::ResolveQuicklyBuf(BYTE *buf, int len)
{
...
BOOL CModBusRtuAndQuickly::ResolveRtuBuf(BYTE *buf, int len)
{
	BYTE temp_flag = 0;
	BYTE CRC_flag = 0;
	WORD wCRC = 0;
 ...
 *--------------------------------------------------------------------------------------
 */
BOOL CModBusRtuAndQuickly::WhetherQuicklyBufValue(BYTE *buf, int &len)
{
	BYTE *pointer = buf;
	WORD wCrc;
	char szBuf[256];
