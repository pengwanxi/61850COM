...
*--------------------------------------------------------------------------------------
*/
BOOL CModBusSCJZ9SY::WhetherBufValue(BYTE *buf, int &len)
{
	BYTE *pointer = buf;
	WORD wCrc;
	char szBuf[256];
...
*--------------------------------------------------------------------------------------
*/
BOOL CModBusSCJZ9SY::ProcessRecvBuf(BYTE *buf, int len)
{
	int i;
	char szBuf[256];
	GetSendPos( );
	...
	case 5:
		...
		break;
	default:
		sprintf(szBuf, "CModBusSCJZ9SY recv no this type\n");
		OutBusDebug(m_byLineNo, (BYTE *)szBuf, strlen(szBuf), 2);
		break;
...
BOOL CModBusSCJZ9SY::ProcessRecvBuf_One(BYTE *buf, int len)
{
	...
	for (i = 0; i <= 9; i++) //�� �� �� Ϣ
	{
		...
		if (i==6) //���й�  
		{
			if((buf_fz[2] & 0x10) == 0)//SIGN ��0~7 λ�ֱ��ʾPa��Pb��Pc��Ps��Qa��Qb��Qc��Qs �ķ��ţ�0��Ϊ���� 1��Ϊ����
			{
				fYcVal = MAKEWORD(buf[4 + i * 2], buf[3 + i * 2]) *0.0001 * pow(10, buf_fz[3]);
				m_pMethod->SetYcData(m_SerialNo, i, fYcVal);
			}
			if((buf_fz[2] & 0x10) == 0x10)//SIGN ��0~7 λ�ֱ��ʾPa��Pb��Pc��Ps��Qa��Qb��Qc��Qs �ķ��ţ�0��Ϊ���� 1��Ϊ����
			{
				fYcVal = -1 * MAKEWORD(buf[4 + i * 2], buf[3 + i * 2]) *0.0001 * pow(10, buf_fz[3]);
				m_pMethod->SetYcData(m_SerialNo, i, fYcVal);
			}
		}
		if (i==7)//���޹�0x02
		{
			if((buf_fz[2] & 0x01) == 0)//SIGN ��0~7 λ�ֱ��ʾPa��Pb��Pc��Ps��Qa��Qb��Qc��Qs �ķ��ţ�0��Ϊ���� 1��Ϊ����
			{
				fYcVal = MAKEWORD(buf[4 + i * 2], buf[3 + i * 2]) *0.0001 * pow(10, buf_fz[3]);
				m_pMethod->SetYcData(m_SerialNo, i, fYcVal);
			}
			if((buf_fz[2] & 0x01) == 0x01)//SIGN ��0~7 λ�ֱ��ʾPa��Pb��Pc��Ps��Qa��Qb��Qc��Qs �ķ��ţ�0��Ϊ���� 1��Ϊ����
			{
				fYcVal = -1 * MAKEWORD(buf[4 + i * 2], buf[3 + i * 2]) *0.0001 * pow(10, buf_fz[3]);
				m_pMethod->SetYcData(m_SerialNo, i, fYcVal);
			}
...
//yc:28-30
BOOL CModBusSCJZ9SY::ProcessRecvBuf_Two(BYTE *buf, int len)
{
	int i, j;
	WORD wYcVal;
	float fYcVal;
...
//yc:55-60
BOOL CModBusSCJZ9SY::ProcessRecvBuf_Three(BYTE *buf, int len)
{
	int i, j;
	WORD wYcVal;
	float fYcVal;
...
//ym��20-27
BOOL CModBusSCJZ9SY::ProcessRecvBuf_Four(BYTE *buf, int len)
{
	int i, j;
	//float ym = 0.0;
	...
	//{
	//	//��һ����ָ���ʼ��
	//	Modbus_HoldReg[0] = ((char*)(&ym)) + 3;	 //�͵�ַָ���λ
	//	Modbus_HoldReg[1] = ((char*)(&ym)) + 2;
	//	Modbus_HoldReg[2] = ((char*)(&ym)) + 1;
	//	Modbus_HoldReg[3] = ((char*)(&ym)) + 0;	 //�ߵ�ַָ���λ
	...
	//	m_pMethod->SetYmData(m_SerialNo, i, ym);
	//}
	...
	{
		//��һ����ָ���ʼ��
		Modbus_HoldReg[0] = ((char*)(&ym)) + 3;
		Modbus_HoldReg[1] = ((char*)(&ym)) + 2;
		Modbus_HoldReg[2] = ((char*)(&ym)) + 1;
		Modbus_HoldReg[3] = ((char*)(&ym)) + 0;
...
// yx��35
BOOL CModBusSCJZ9SY::ProcessRecvBuf_Five(BYTE *buf, int len)
{
	BYTE byYxByte;
	BYTE byYxValue;
	for (int i = 0; i<7; i++)
...
*---------------------------------------------------------------------------------------
*/
BOOL CModBusSCJZ9SY::GetProtocolBuf(BYTE *buf, int &len, PBUSMSG pBusMsg)
{
	char szBuf[256] = "";
...
*--------------------------------------------------------------------------------------
*/
BYTE CModBusSCJZ9SY::GetSendPos(void)
{
...
*--------------------------------------------------------------------------------------
*/
void CModBusSCJZ9SY::ChangeSendPos(void)
{
...
BOOL CModBusSCJZ9SY::ProcessProtocolBuf(BYTE *buf, int len) 
{
...
*--------------------------------------------------------------------------------------
*/
BOOL CModBusSCJZ9SY::Init(BYTE byLineNo)
{
	return TRUE;
...
*---------------------------------------------------------------------------------------
*/
void CModBusSCJZ9SY::TimerProc(void)
{
	if (m_bySendCount > 3 || m_byRecvCount > 3)
	{
		...
		if (m_bLinkStatus)
		{
			...
			OutBusDebug(m_byLineNo, (BYTE *)"CModBusSCJZ9SY:unlink\n", 30, 2);
		}
	}
...
*--------------------------------------------------------------------------------------
*/
BOOL CModBusSCJZ9SY::GetDevCommState(void)
{
	if (m_bLinkStatus)
	{
...
}		/* -----  end of method CModBusSCJZ9SY::GetDevCommState  ----- */
