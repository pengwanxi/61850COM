...
BOOL CModBusWSCGQ::GetProtocolBuf(BYTE * buf, int &len, PBUSMSG pBusMsg /*= NULL*/)
{
	char szBuf[256] = "";
	...
	default:
		{
			sprintf(szBuf, "%s", "CModBusWSCGQ send buf err !!!\n");
...
BOOL CModBusWSCGQ::ProcessProtocolBuf(BYTE * buf, int len)
{
...
BOOL CModBusWSCGQ::ProcessRecvBuf(BYTE *buf, int len)
{
	int i;
	char szBuf[256];
	...
	case 1:
		...
		break;
	default:
		sprintf(szBuf, "CModBusWSCGQ recv no this type\n");
		OutBusDebug(m_byLineNo, (BYTE *)szBuf, strlen(szBuf), 2);
		break;
...
BOOL CModBusWSCGQ::ProcessRecvBuf_yc(BYTE *buf, int len)
{
	BYTE * pBuf = buf;
	BYTE byAddr = pBuf[ 0 ] ;
	BYTE byFunc = pBuf[1];
...
*--------------------------------------------------------------------------------------
*/
BOOL CModBusWSCGQ::WhetherBufValue(BYTE *buf, int &len)
{
	BYTE *pointer = buf;
	WORD wCrc;
	char szBuf[256];
...
BOOL CModBusWSCGQ::GetDevCommState()
{
	return false;
}
