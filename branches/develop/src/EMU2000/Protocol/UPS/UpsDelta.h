...
#define UPSMASTER_KESHIDA_MAX_POX			3		//报文发送类别最多个数


class CUpsDelta : public CProtocol_UpsMaster
{
public:
	CUpsDelta();
	virtual ~CUpsDelta();
	...
	BOOL UPSQueryProtocolyc(BYTE * buf , int &len);

	BOOL UPSQueryBatteryVoltage(BYTE * buf , int &len);
	...
	BOOL UPSParseProtocolyc(BYTE *buf, int len);
	...
	BOOL UPSParseBatteryVoltage(BYTE *buf, int len);
