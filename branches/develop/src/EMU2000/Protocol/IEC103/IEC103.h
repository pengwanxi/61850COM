 ...
 * =======================
 */
#ifndef  _IEC103_H__INC
#define  _IEC103_H__INC
...
typedef enum _SENDSTATUS
{
	...
	/* 变化帧*/
	C_SYN_Ta_3, //时间同步 ASDU6
	C_IGI_NA_3, //总查询   ASDU7
	C_CI_NA_3,  //召唤遥脉 ASDU88
	C_GD_NA_3,  //通用分类数据 ASDU10
	C_GRC_NA_3, //一般命令 ASDU20
	C_GC_NA_3,  //通用命令 ASDU21
	C_ODT_NA_3, //扰动传输命令 ASDU24
	C_ADT_NA_3, //扰动数据认可 ASDU25

}SENDSTATUS;				/* ----------  end of enum SendStatus  ---------- */
 ...
 * =====================================================================================
 */
class CIEC103:public CProtocol_IEC103
{
public:
    /* ====================  LIFECYCLE     ======================================= */
    CIEC103 ();                             /* constructor      */
...
private:
    ...
    vector < CfgInfo > m_IEC103_CfgInfo; //配置信息
...
public:
    ...
    //显示传送原因
    void DisplayCot( BYTE byCot );
    //获取点号
    BOOL GetModulePnt( BYTE byDataType, BYTE byFunType, BYTE byInfoIndex, WORD &wPnt );
    //获取配置信息 fun inf
    BOOL GetModuleInfo( BYTE byDataType, WORD wPnt, CfgInfo &tCfgInfo, BYTE &byFunType, BYTE &byInfoIndex );
...
private:
	...
	//复位帧计数单位
    BOOL ResetFrameCountBit( BYTE *buf, int &len );
    //复位通信单元
    ...
    //请求链路状态
    BOOL RequestLinkState(BYTE *buf, int &len);


    /* 可变帧 */
    //时间同步
    BOOL TimeSyn(BYTE *buf, int &len);
    //总召唤
    BOOL TotalCallData(BYTE *buf, int &len);
    //召唤遥脉
    ...
    /* 遥控 */
    //选择
    BOOL YkSel( YK_DATA *pYkData, BYTE *buf, int &len );
    //执行
    BOOL YkExct( YK_DATA *pYkData, BYTE *buf, int &len );
    //取消
    BOOL YkCancel( YK_DATA *pYkData, BYTE *buf, int &len );
    ...
    /* 固定帧 */
    //确认帧
    BOOL ConfigFrame(BYTE *buf, int len);
    //忙帧
    BOOL BusyFrame(BYTE *buf, int len);
    //无所要求的数据帧回答
    BOOL NoneRequireFrame(BYTE *buf, int len);
    //链路工作正常帧
    BOOL LinkRunFrame(BYTE *buf, int len);
    //链路服务未工作帧
    BOOL LinkNoneRunFrame(BYTE *buf, int len);
    //链路服务未完成帧
    BOOL LinkUnFinishFrame(BYTE *buf, int len);

    /* 可变帧 */
    //标识报文 ASDU5
    BOOL M_IRCFS_TA_3_Frame(BYTE *buf, int len);
    //时间同步 ASDU6
    BOOL C_SYN_TA_3_Frame(BYTE *buf, int len);
    //总查询结束 ASDU8
    BOOL M_TGI_NA_3_Frame(BYTE *buf, int len);
    ...
    /* 遥测 */
    //被测值 ASDU3
    BOOL M_MEI_NA_3_Frame(BYTE *buf, int len);
    //带相对时间的有时标被测值 ASUD4
    BOOL M_TME_TA_3_Frame( BYTE *buf, int len );
    //被测值II ASDU9
    BOOL M_MEII_NA_3_Frame(BYTE *buf, int len);
    //通用分类数据 ASDU10		特殊开发 by cyz!
    BOOL M_GD_NA_3_Frame(BYTE *buf, int len);
    //被测值III ASDU15
    BOOL M_MEIII_NA_3_Frame(BYTE *buf, int len);
    //被测值III ASDU32
    BOOL M_MEIII_TA_3_Frame(BYTE *buf, int len);
    //被测值IV ASDU33
    BOOL M_MEIV_TA_3_Frame(BYTE *buf, int len);
    //被测值向量V ASDU34
    BOOL M_MEV_TA_3_Frame(BYTE *buf, int len);
    //带时标的被测值向量VI ASDU35
    BOOL M_MEVI_TA_3_Frame(BYTE *buf, int len);
    //响应总召唤的被测值VII 超过门限值得被测值II ASDU50
    BOOL M_MEVII_NA_3_Frame(BYTE *buf, int len );

    //遥脉
    //电能脉冲计数量帧 ASDU36
    BOOL M_IT_NA_3_Frame ( BYTE *buf, int len );
    //带时标的电能脉冲计数量帧  ASDU37
    BOOL M_IT_TA_3_Frame ( BYTE *buf, int len );
public:
    // 西门子的遥脉上送
    ...
    /* 遥控 */
    //遥控返回
    BOOL C_DC_NA_3_Frame(BYTE *buf, int len );

    float floatvalue(BYTE *);
    //???????? ASDU65
    //OL C_RC_NA_3_Frame(BYTE *buf, int len );
};			/* -----  end of class CIEC103  ----- */
