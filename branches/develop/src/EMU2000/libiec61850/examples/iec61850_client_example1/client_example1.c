...
{/*{{{*/
	...
	IedConnection_connect(con, &error, hostname, tcpPort);	//建立连接!	这里有对recv的调用! 读取的内容存在:con->connection->isoClient->CotpConnection->readBuffer->buffer中!	如何判断连接是否断开?
	...
	if (error == IED_ERROR_OK) {			//正常连接!
		...
		//其名曰：读对象，然而读的却是一个普通数字，“对象”者，何谓也?	王曰：对象既可以是具体的数字，也可以是结构体，甚至是数组,就看BAY1LD0/后面路径的等级!即如果phsA后面省略则value的值就是一个phsA结构体!			//正常读取!*/
		/*MmsValue* value = IedConnection_readObject(con, &error, "BAY1LD0/CMMXU1.A.phsB.cVal.mag.f", IEC61850_FC_MX);
		
		//其名曰：读对象，然而读的却是一个普通数字，“对象”者，何谓也?	王曰：对象既可以是具体的数字，也可以是结构体，甚至是数组,就看BAY1LD0/后面路径的等级!即如果phsA后面省略则value的值就是一个phsA结构体!*/
		/*MmsValue* value = IedConnection_readObject(con, &error, "BAY1LD0/CMMXU1.A.phsB", IEC61850_FC_MX);	
			
	    //其名曰：读对象，然而读的却是一个普通数字，“对象”者，何谓也?	王曰：对象既可以是具体的数字，也可以是结构体，甚至是数组,就看BAY1LD0/后面路径的等级!即如果phsA后面省略则value的值就是一个phsA结构体!
		MmsValue* value = IedConnection_readObject(con, &error, "BAY1LD0/CMMXU1", IEC61850_FC_NONE);
