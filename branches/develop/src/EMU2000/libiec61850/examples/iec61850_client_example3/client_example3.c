...
{/*{{{*/
	...
	if (error == IED_ERROR_OK) {
		...
		ControlObjectClient control
			...
			= ControlObjectClient_create("BAY1CTRL/CBCSWI1.Pos", con);

		/*MmsValue* ctlVal = MmsValue_newBoolean(true);*/
		MmsValue* ctlVal = MmsValue_newBoolean(false);			//测试时使用false，使用true不成功!

		ControlObjectClient_setOrigin(control, NULL, 3);

		if(ControlObjectClient_selectWithValue(control, ctlVal))						//+4 by cyz!
			printf("\n\nControlObjectClient_selectWithValue successfully!\n\n");
		else
			printf("\n\nControlObjectClient_selectWithValue failed!\n\n");

		if (ControlObjectClient_operate(control, ctlVal, 0 /* operate now */))	//凡涉及对服务器之写操作，全部失败，我心伤悲，莫知我哀!
		...
		MmsValue* stVal = IedConnection_readObject(con, &error, "BAY1CTRL/CBCSWI1.Pos.stVal", IEC61850_FC_ST);
		/*MmsValue* stVal = IedConnection_readObject(con, &error, "BAY1CTRL/CBCSWI1.Pos.ctlVal", IEC61850_FC_CO);*/

		if (error == IED_ERROR_OK) {
			if(MmsValue_getType(stVal) == MMS_BOOLEAN){			//+ by cyz!
				bool state = MmsValue_getBoolean(stVal);

				/*printf("New status of simpleIOGenericIO/GGIO1.SPCSO1.stVal: %i\n", state);*/
				printf("New status of BAY1CTRL/CBCSWI1.Pos.stVal: %i\n", state);
			}
