...
#include "ber_decode.h"
	...
	int
BerDecoder_decodeLength(uint8_t* buffer, int* length, int bufPos, int maxBufPos)	//第一次:bufPos:6, maxBufPos:10 第二次:bufPos:8, maxBufPos:10
{/*{{{*/
	if (bufPos >= maxBufPos)
		return -1;

	uint8_t len1 = buffer[bufPos++];				//len1:3
...
/*
 * descriptor:将para0[para2]到para0[para1 + para2 - 1]的内容化为整数返回!
 */
	uint32_t
