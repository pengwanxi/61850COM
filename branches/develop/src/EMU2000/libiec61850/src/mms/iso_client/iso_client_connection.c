...
{/*{{{*/
	...
	while (true) {
		...
		if (!IsoPresentation_parseUserData(self->presentation, IsoSession_getUserData(self->session))) {			//将para1经过处理写入para0!
			if (DEBUG_ISO_CLIENT)
	...
	self->callback(ISO_IND_CLOSED, self->callbackParameter, NULL);								//监视连接是否中断!
...
{/*{{{*/
	...
	if (!Socket_connect(self->socket, params->hostname, params->tcpPort))		//创建socket连接!
		goto returnError;
...
{/*{{{*/
	...
	CotpIndication indication = CotpConnection_sendDataMessage(self->cotpConnection, sessionBuffer);	//实际发送的buf:para0->writeBuffer->buffer!
	/*printf("******** file:%s indication:%d ********\n", __FILE__, indication);*/
...
/*
 * description	:为para中的某字段初始化!
 * parameter	:struct!
 * return		:被赋值的字段!
 */
ByteBuffer* IsoClientConnection_allocateTransmitBuffer(IsoClientConnection self)
{/*{{{*/
