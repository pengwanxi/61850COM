...
/*
 * description:
 * 		Set a single bit (set to one) of an MmsType object of type
 * 		MMS_BITSTRING.
 * parameters:
 * 		self:	MmsValue instance to operate on.Has to be of a type
 * 		MMS_BITSTRING.
 * 		bitPos:	the position of the bit in the bit string.Starting with 0.The
 * 		bit with position 0 is the first bit if the MmsValue instance is
 * 		serialized.
 * 		value:	the new value if the bit (true = 1 / false = 0)
 */
	void
 ...
 * 		The integer representation in the bit string assumes the first bit is
 * 		the least significant bit(little endian bit order).
 * parameters:
 * 		self:	MmsValue instance to operate on.Has to be of a type
 * 		MMS_BIT_STRING.
...
{/*{{{*/
	if (self->type == MMS_FLOAT) {
		if (self->value.floatingPoint.formatWidth == 32) {
			...
			/*memcpy(buf, self->value.floatingPoint.buf, 4);*/
 ...
 * 		This operation frees all dynamically allocated memory of the MmsValue
 * 		instance.If the instance is of type MMS_STRUCTURE or MMS_ARRAY all child
 * 		elements will be deleted too.
 * parameters:
 ...
 * 		This function will allocate as much memory as required to hold the
 * 		string and sets the maximum size of the string to this size.
 * parameters:
 * 		string	a text string that should be the value of the new instance of
