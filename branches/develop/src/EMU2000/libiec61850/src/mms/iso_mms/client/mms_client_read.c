...
/*
 * 调试记录：elementCount = 1, errorCode = 1
 */
				value = MmsValue_newDataAccessError(dataAccessError);
				/*
				 * 调试记录: value->type = MMS_DATA_ACCESS_ERROR,
				 * value->value.dataAccessError = DATA_ACCESS_ERROR_UNKNOWN;
				 */
			}/*}}}*/
			else
	...
	if (valueList == NULL)
		valueList = value;
	/*
	 * 调试记录: valueList->type = MMS_DATA_ACCESS_ERROR, valueList->value.dataAccessError = 1
	 */
...
{/*{{{*/
	...
	asn_dec_rval_t rval = ber_decode(NULL, &asn_DEF_MmsPdu,
			(void**) &mmsPdu, ByteBuffer_getBuffer(message), ByteBuffer_getSize(message));	//mmsPdu:是如何由空变得充实的是我国现阶段面临的主要矛盾!
	...
	if (mmsPdu->present == MmsPdu_PR_confirmedResponsePdu) {
		...
		if (mmsPdu->choice.confirmedResponsePdu.confirmedServiceResponse.present == ConfirmedServiceResponse_PR_read) {
			...
			valueList = mmsClient_parseListOfAccessResults(response->listOfAccessResult.list.array,
					elementCount, createArray);	//para0:mmsPdu->choice.confirmedResponsePdu.confirmedServiceResponse.choice.read->listOfAccessResult.list.array!
		}
	}
...
{/*{{{*/
	mmsPdu->choice.confirmedRequestPdu.confirmedServiceRequest.present =
		ConfirmedServiceRequest_PR_read;			//ConfirmedServiceRequest_PR_read:枚举，值为2!
...
{/*{{{*/
	...
	rval = der_encode(&asn_DEF_MmsPdu, mmsPdu,						//asn_DEF_MmsPdu:在哪里被赋值的?	曰:asn_DEF_MmsPdu是一个结构体，被定义的时候已经初始化了!
			(asn_app_consume_bytes_f*) mmsClient_write_out, (void*) writeBuffer);
