...
/*
 * description:
 * 		获取para字符串中$字符前之string之长度，若不存在$则返回整个para之长度!
 */
	static size_t
 ...
 * 		nameId:	the relative MMS name to the child MMS variable (with "$"
 * 		separator!)
 * returns:
...
{/*{{{*/
	const char* separator = strchr(nameId, '$');		//strchr:从para0中搜寻para1第一次出现的位置!
