...
{/*{{{*/
	...
	if (self->rawMmsMessageHandler != NULL) {				//没有执行!
		MmsRawMessageHandler handler = (MmsRawMessageHandler) self->rawMmsMessageHandler;		//rawMmsMessageHandler在哪里被赋值的?根据cscope查找的结果似乎不对!除非启动另一个进程，但是源码中找不到创建进程的关键字fork/system!
		handler(self->rawMmsMessageHandlerParameter, message->buffer, message->size, false);
	}
#endif /* (CONFIG_MMS_RAW_MESSAGE_LOGGING == 1) */

	IsoClientConnection_sendMessage(self->isoClient, message);				//发送消息，使用socket!实际使用的buf:para0->cotpConnection->writeBuffer->buffer!
	...
	while (currentTime < waitUntilTime) {
		...
		if (receivedInvokeId == invokeId) {
			...
			receivedMessage = self->lastResponse;				//在哪里响应的?		曰:MmsConnection_create-->IsoClientConnection_create中被回调函数注册! 调用发生在新线程中，流程如下:IedConnection_connect-->MmsConnection-->IsoClientConnection_associate-->connectionThreadFunction-->connectionHandlingThread.
...
/*
 * Desc:
 * para:	para0:连接对象，para1:函数指针/回调函数，para2:
 * retu:	void
 */
	void
 ...
 * parameters	:para0:IED对象之连接超时元, para1:欲设之超时时间
 * return		:void!
 */
	void
...
{/*{{{*/
	uint64_t currentTime = Hal_getTimeInMs();			//获取系统时间，单位ms!

	uint64_t waitUntilTime = currentTime + self->requestTimeout;

	while (currentTime < waitUntilTime) {					//在这里等1s，等着connectionState被置位，我同意你等，但是你也应该开启一枚线程给connectionState置位啊!
		if (self->connectionState != MMS_CON_WAITING)
			break;

		Thread_sleep(10);			//休息10ms!
...
{/*{{{*/
	...
	bool moreFollows = mmsClient_getNameListSingleRequest(&list, self, mmsError, domainId,
 ...
 * 		domainId:	the domain name of the variable to be read or NULL to read a
 * 		VMD specific: named variable
 * 		itemId:	name of the variable to be read.
 * return:
 * 		returns a MmsValue object or NULL if the request failed.The MmsValue
 * 		object can either be a simple value or a complex value or array.It is
 * 		also possible that the return value is NULL even if mmsError =
 * 		MMS_ERROR_NONE this is the case when the servers returns an empty result
 * 		list.
 */
	MmsValue*
MmsConnection_readVariable(MmsConnection self, MmsError* mmsError,
		const char* domainId, const char* itemId)	//调试记录:domainId:SampleIEDDevice1 itemId:DGEN1$MX$MX$TotWh$mag$h
{/*{{{*/
	MmsValue* value = NULL;
	...
	ByteBuffer* payload = IsoClientConnection_allocateTransmitBuffer(self->isoClient);	//初始化para->transmitPayloadBuffer!
...
{/*{{{*/
	...
	*mmsError = MMS_ERROR_NONE;
	...
	ByteBuffer* responseMessage = sendRequestAndWaitForResponse(self, invokeId, payload);			//self->lastResponseError:MMS_ERROR_SERVICE_TIMEOUT!
	...
	*mmsError = MMS_ERROR_NONE;
	...
	if (self->lastResponseError != MMS_ERROR_NONE){
		...
		*mmsError = self->lastResponseError;
		...
		retVal = mmsClient_parseWriteResponse(self->lastResponse, self->lastResponseBufPos, mmsError);				//*mmsError:83
	printf("file:%s line:%d mmsError:%d\n", __FILE__, __LINE__, *mmsError);
 ...
 * 		The parameter accessResults is a pointer to a LinkedList reference.The
 * 		methods will create a new LinkedList object that contains the
 * 		AccessResults of the single variable write attempts.It is up to the user
 * 		to free this objects properly (e.g. with
 * 		LinkedList_destroyDeep(accessResults,MmsValue_delete)).
 * Parameters:
 * 		self:	MmsConnection instance to operate on
 * 		mmsError:	user provided to store error code.
 * 		domainId:	the common domain name of all variables to be written.
 * 		items:	a linked list containing the names of the variables to be
 * 		written.The names are C strings.
 * 		values:	values of the variables to be written.
 * 		(OUTPUT)	the MmsValue objects of type MMS_DATA_ACCESS_ERROR
 * 		representing the write success of a single variable write.
 */
	void
