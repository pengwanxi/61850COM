...
_search4tag(const void *ap, const void *bp) {/*{{{*/
	...
	int a_class = BER_TAG_CLASS(a->el_tag);		//a_class = para & 3!
...
CHOICE_decode_ber(asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	case 1:
		 ...
		 */
		tag_len = ber_fetch_tag(ptr, LEFT, &tlv_tag);				//para0 = 161, para2 = 6, tag_len = 1;
		...
		do {
			...
			t2m = (asn_TYPE_tag2member_t *)bsearch(&key,
					...
					sizeof(specs->tag2el[0]), _search4tag);		//bsearch:二分查找之系统函数!
			if(t2m) {
				/*
	...
	case 2:
		...
		do {
			 ...
			 */
			//调用constr_SEQUENCE.c:SEQUENCE_decode_ber! 第一次: opt_codec_ctx->max_stack_size = 30000, elm->type = , memb_ptr2 = 指向自己函数指针!, ptr = , LEFT = -1, elm->tag_mode = -1;
			rval = elm->type->ber_decoder(opt_codec_ctx, elm->type, memb_ptr2, ptr, LEFT, elm->tag_mode);
			switch(rval.code) {
			case RC_OK:
				break;
