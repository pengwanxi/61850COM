...
exit_with_error:
	*mmsError = MMS_ERROR_PARSING_RESPONSE;
	...
	MmsDataAccessError
mmsClient_parseWriteResponse(ByteBuffer* message, int32_t bufPos, MmsError* mmsError)			//bufPos:5
{/*{{{*/
	MmsDataAccessError retVal = DATA_ACCESS_ERROR_UNKNOWN;

	uint8_t* buf = message->buffer;
	int size = message->size;						//size:10
	...
	if (tag == 0xa5) {/*{{{*/
		...
		if (tag == 0x80) {
			...
			if (dataAccessErrorCode < 13) {
				*mmsError = mapDataAccessErrorToMmsError(dataAccessErrorCode);			//debug note:dataAccessErrorCode:3
