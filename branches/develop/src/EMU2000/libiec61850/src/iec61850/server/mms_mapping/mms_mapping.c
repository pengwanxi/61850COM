 ...
 *																	|----para1			(para1 != NULL)
 *																	|
 * desc:	检索para0中是否存在字符'/'如果存在且位于前64byte则返回--
 * 																	|
 * 																	|----'/'前之string	(para1 == NULL)
 * para:	para0:待检测stirng，para1:
 * retu:	其他情况返回NULL!
 */
	char*
...
/*
 * 											
 * desc:获取para0中'/'后string，某些情况下要经过处理!
 * para:para0:待分析string, para1:功能约束，para2:返回string的依据!
 * 																|---- para2		(para2 != NULL)
 * 																|
 *							|---- para1 == IEC61850_FC_NONE ---- ---- 第一个'/'后之string	(para2 == NULL)
 *							|
 * retu:				----
 * 							|									|---- '/'后string+三枚字符!	(没有'.')
 * 							|---- or ---- para0 '/'之后string-------- '.'之后插入几枚字符		('.'存在)
 */
	char*
...
{/*{{{*/
	...
	if (fc == IEC61850_FC_NONE) {/*{{{*/
		...
		strcpy(mmsVariableName, objectReference + i);	//这样真的健康吗! 如果buffer != NULL且其长度大于objectReference + i当如何!
	...
	int namePartLength = objRefLength - i - 1;	//'/'之后位置!
