...
struct sClientReport				//report和reportcontrolblock的关系!
{/*{{{*/
	ReportCallbackFunction callback;
	void* callbackParameter;
	char* rcbReference;
	char* rptId;					//report ID! What's your function?

	char* dataSetName;				//数据集之姓名!
	int dataSetNameSize; /* size of the dataSetName buffer */

	MmsValue* entryId;				//入口id
	MmsValue* dataReferences;
	MmsValue* dataSetValues;
	ReasonForInclusion* reasonForInclusion;	//原因码! 决定了报告之触发原因!
	...
	uint64_t timestamp;
	uint16_t seqNum;				//顺序编号
	uint32_t confRev;
	bool bufOverflow;				//是否发生缓冲溢出!
};/*}}}*/
...
/*
 * description:
 * 		Install a report handler function for the specified report control
 * 		block(RCB).
 * 		This function will replace a report handler set earlier for the
 * 		specified RCB.The report handler will be called whenever a report for
 * 		the specified RCB is received.Please note that this function should be
 * 		called whenever the RCB data set is changed or updated.Otherwise the
 * 		internal data structures storing the received data set values will not
 * 		be updated correctly.
 * 		When replacing a report handler you only have to call this function.There
 * 		is no separate call to IedConnection_uninstallReportHandler() required.
 * parameters:
 * 		self:	the connection object
 * 		rcbReference:	object reference of the report control block.
 * 		rptId:	a string that identifies the report.If the rptId is not
 * 		available then the rcbReference is used to identify the report.
 * 		handler:	user provided callback function to be invoked when a report
 * 		is received.
 * 		handlerParameter:	user provided parameter that will be passed to the
 * 		callback function.
 */
	void
