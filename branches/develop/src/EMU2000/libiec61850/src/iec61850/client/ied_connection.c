 ...
 * 		Note:A ClientDataset cannot be created directly by the application but
 * 		only by the IedConnection_readDataSetValues function.Therefore there is
 * 		no public ClientDataSet_create function.
 * parameters:
 ...
 * 		This function creates a new IedConnection instance that is used to handle a
 * 		connection to an IED. It allocated all required resources.The new connection
 * 		is in the "idle" state.Before it can be used the connect method has to be
 * 		called.
 *
 * return:
...
{/*{{{*/
	...
	self->enabledReports = LinkedList_create();			//report是什么概念?
 ...
 * 		This function can be used to determine if the connection is established or
 * 		closed.
 * parameters:
...
/*
 * description	:设置IED连接状态
 * parameter	:para0:ied对象，para1:欲设置之IED状态
 * return		:void
 */
	static void
...
{/*{{{*/
	...
	if (IedConnection_getState(self) != IED_STATE_CONNECTED) {
		...
		if (MmsConnection_connect(self->connection, &mmsError, hostname, tcpPort)) {
			*error = IED_ERROR_OK;		//枚举的用法不同于结构体!
 ...
 * 		This will close the MMS association by sending an ACSE abort message to
 * 		the server.After sending the abort message the connection is closed
 * 		immediately.The client can assume the connection to be closed when the
 * 		function returns and the destroy method can be called.If the connection
 * 		is not in "connected" state an IED_ERROR_NOT_CONNECTED error will be
 * 		reported.
 * parameters:
 * 		self	the connection object
 * 		error	the error code if an error occurs.
 ...
 * 		The connection will be closed if it is in "connected" state. All allocated
 * 		resources of the connection will be freed.
 * parameters:
 * 		self	the connection object.
 */
	void
...
/*
 * description:
 * 		return the MMS variable type specification of the data attribute
 * 		referenced by dataAttributeReference and function constraint fc.
 * 		This function can be used to get the MMS variable type specification for
 * 		an IEC61850 data attribute.It is an extension of the ACSI that may be
 * 		required by generic client application.
 * parameters:
 * 		self	the connection object
 * 		error	the error code if an error occurs
...
/*
 * description:
 * 		read a functional constrained data attribute (FCDA) or functional
 * 		constrained data(FCD).
 * parameters:
 * 		self	the connection object to operate on.
 * 		error	the error code if an error occurs.
 * 		object	reference of the object/attribute to read
 * 		fc		the functional constraint of the data attribute or data object
 * 		to read.
 * return:
 * 		the MmsValue instance of the received value or NULL if the request failed.
 */
	MmsValue*
IedConnection_readObject(IedConnection self, IedClientError* error, const char* objectReference, FunctionalConstraint fc)
{/*{{{*/
	char domainIdBuffer[65] = "";
	...
	domainId = MmsMapping_getMmsDomainFromObjectReference(objectReference, domainIdBuffer);			//获取para0'/'之前string		//domainId:SampleIEDDevice1
	itemId = MmsMapping_createMmsVariableNameFromObjectReference(objectReference, fc, itemIdBuffer);//获取para0'/'之后string并插入功能约束!	itemId:DGEN1$MX$TotWh$mag$f
...
{/*{{{*/
	...
	if (value != NULL) {
		...
		/*else if(MmsValue_getType(value) == MMS_BOOLEAN)*/			//+2 by cyz!
			/*retVal = (value->value.boolean == true) ? 1 : 0;*/
		else {
			if (MmsValue_getType(value) == MMS_DATA_ACCESS_ERROR)
				*error = iedConnection_mapDataAccessErrorToIedError(MmsValue_getDataAccessError(value));
			else
...
/*
 * description:
 * 		write a functional constrained data attribute(FCDA) or functional
 * 		constrained data(FCD).
 * parameters:
 * 		self	the connection object to operate on
 * 		error	the error code if an error occurs
 ...
 * 		value	the MmsValue to write (has to be of the correct type -
 * 		MMS_STRUCTURE for FCD).
 */
	void
...
{/*{{{*/
	...
	*error = iedConnection_mapMmsErrorToIedError(mmsError);		//error take place here! mmsError = MMS_ERROR_DEFINITION_TYPE_INCONSISTENT = 33
}/*}}}*/
	...
	*error = IED_ERROR_OK;
	...
	if (self->logicalDevices != NULL) {
		...
		while (logicalDevice != NULL) {
			...
			char* logicalDeviceName = StringUtils_copyString(icLogicalDevice->name);			//malloc内存，将para纳入，返回malloc指针!

			LinkedList_add(logicalDeviceList, logicalDeviceName);								//将逻辑设备之名纳入逻辑设备链表data成员!
 ...
 * 		GetServerDirectory ACSI service implementation.This function will either
 * 		return the list of logical devices (LD) present at the server or the
 * 		list of available files.
 * parameters:
 * 		self	the connection object.
 * 		error	the error code if an error occurs.
 * 		getFileNames	get list of files instead of logical device names (TO BE
 * 		IMPLEMENTED)
 * return:
 * 		LinkedList with string elements representing the logical device names or
 * 		file names.
 */
	LinkedList /*<char*>*/
 ...
 * 		Get access to the underlying MmsConnection instance used by this
 * 		IedConnection.This can be used to set/change specific MmsConnection
 * 		parameters or to call low-level MMS services/functions
 * parameters:
 *  	self the connection object
 * returns:
