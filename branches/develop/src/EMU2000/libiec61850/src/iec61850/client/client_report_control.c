...
/*
 * description	:若para中存在".B"字串
 * parameter	:string
 * return		:true for 存在，false for 不存在!
 */
static bool isBufferedRcb(const char* objectReference)
{/*{{{*/
	const char* separator = strchr(objectReference, '.');	//strchr:查找para0中para1首次出现的位置!
...
{/*{{{*/
	...
	self->objectReference = StringUtils_copyString(objectReference);	//将para存入malloc再返回malloc指针!
...
{/*{{{*/
	...
	void
ClientReportControlBlock_setOptFlds(ClientReportControlBlock self, int optFlds)	//未被调用!
{/*{{{*/
...
/*
 * description:
 * 设置报告的触发条件，使用TRG_OPT_DATA_CHANGED,TRG_OPT_QUALITY_CHANGED,TRG_OPT_DATA_UPDATE,TRG_OPT_INTEGRITY,TRG_OPT_GI宏设置条件参数，中间可以使用or运算设置多个触发条件!
 */
	void
...
{/*{{{*/
	int rcbElementCount = MmsValue_getArraySize(values);				//rcbElementCount:13

	updateOrClone(&(self->rptId), values, 0);							//self之objectReference&isBuffered不为空，其它全为空!
	updateOrClone(&(self->rptEna), values, 1);							//what are you doing?
	...
	if (self->isBuffered) {/*{{{*/
		...
		updateOrClone(&(self->timeOfEntry), values, 12);				//这十三钗是报告控制块结构体成员，同时也是report之所有字段，且顺序是依次对应的!
 ...
 * description:
		...
		"simpleIOGenericIO/LLN0.RP.EventsRCB01" or
...
{/*{{{*/
	*error = IED_ERROR_OK;
	...
	bool isBuffered = ClientReportControlBlock_isBuffered(rcb);				//是否缓冲，非缓冲者异常重新连接则丢失，缓存者，存续也!
	bool sendGILast = false; /* GI should be sent last when RptEna=TRUE is included */			//如果不包含GI当如何?

	char domainId[65];
	char itemId[129];

	char* rcbReference = ClientReportControlBlock_getObjectReference(rcb);		//获取report路径/姓名!这里为BAY1LD0/LLN0.BR.rcbMeasFlt01

	MmsMapping_getMmsDomainFromObjectReference(rcbReference, domainId);

	strcpy(itemId, rcbReference + strlen(domainId) + 1);

	StringUtils_replace(itemId, '.', '$');			//.替换为$
	...
	if (parametersMask & RCB_ELEMENT_TRG_OPS) {/*{{{*/
		...
		LinkedList_add(itemIds, StringUtils_copyString(itemId));
		LinkedList_add(values, rcb->trgOps);				//rcb->trgOps:dataAccessError:DATA_ACCESS_ERROR_TYPE_UNSUPPORTED!
	}/*}}}*/

	if (parametersMask & RCB_ELEMENT_INTG_PD) {/*{{{*/
		strcpy(itemId + itemIdLen, "$IntgPd");				//rcb->intgPd:dataAccessError:异常!
	...
	if (parametersMask & RCB_ELEMENT_RPT_ENA) {/*{{{*/
		...
		LinkedList_add(itemIds, StringUtils_copyString(itemId));
		LinkedList_add(values, rcb->rptEna);				//rcb->rptEna:dataAccessError:DATA_ACCESS_ERROR_HARDWARE_FAULT!
	}/*}}}*/
