...
{/*{{{*/
	...
	uint32_t ctlModel = IedConnection_readUnsigned32Value(connection, &error, reference, IEC61850_FC_CF);		//为什么要读取应该是为了验证是否联通!
	...
	if (MmsVariableSpecification_getType(ctlVarSpec) == MMS_STRUCTURE) {
		...
		if (oper)
		{
			...
			MmsVariableSpecification* operTm = MmsVariableSpecification_getNamedVariableRecursive(oper, "operTm");	//operTm:未之闻也!是不是timeout?
 ...
 * parameters:
 * 		self:	the contrl object instance to use
 * 		ctlVal:	the control value (for APC the value may be either AnalogueValue
 ...
 * 		ctlVal:	the control value(for APC the value may be either
 * 		AnalogueValue(MMS_STRUCTURE) or MMS_FLOAT/MMS_INTEGER)
 * return:
 * 		true if select has been successful,false otherwise.
 */
	bool
...
{/*{{{*/
	...
	if (mmsError != MMS_ERROR_NONE) {				//error take place here!
		if (DEBUG_IED_CLIENT)
