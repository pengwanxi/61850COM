...
{/*{{{*/
	...
	if((self->fd = socket(AF_INET, SOCK_STREAM, 0)) == -1){					//为什么getsockopt判断该socket有问题!是不是和socket类型有关呢?
	...
	if (select(self->fd + 1, NULL, &fdSet , NULL, &timeout) == 1) {
		...
		getsockopt(self->fd, SOL_SOCKET, SO_ERROR, &so_error, &len);			//error take place here! 错误原因：忘了命令行指定ip!	由socket返回错误引起的trouble! so_error:0 for nothing, others for error!
